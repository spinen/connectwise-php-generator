...
ls -A "${SPECS_DIR}" | while read -r spec_file; do
    ...
    POPER_NAME="$(echo "${NAME:0:1}" | tr  '[a-z]' '[A-Z]')${NAME:1}";
    NAME_SPACE="Spinen\ConnectWise\Clients\\${POPER_NAME}";
    ESCAPED_NAME_SPACE="Spinen\\\\\\\\ConnectWise\\\\\\\\Clients\\\\\\\\${POPER_NAME}\\\\\\\\";
    REPO="connectwise-${NAME}-php-client";
    REPO_PATH="${GEN_DIR}${REPO}/";

    echo "Generating API for ${POPER_NAME}";
    ...
    java -jar ${SWAGGER_DIR}swagger-codegen-master/modules/swagger-codegen-cli/target/swagger-codegen-cli.jar generate \
        --additional-properties \
            packagePath="${REPO}",composerVendorName="spinen",composerProjectName="${REPO}",description="Spinen's PHP ConnectWise Client for ${POPER_NAME} API generated by Swagger Code Generator." \
